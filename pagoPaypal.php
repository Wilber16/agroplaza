...
<?php
   $sql = "SELECT total_price FROM orders WHERE user_id = ? ORDER BY id DESC LIMIT 1";
   $select_total_price = $conn->prepare($sql);
   $select_total_price->execute([$user_id]);
   $fetch_total_price = $select_total_price->fetch(PDO::FETCH_ASSOC);
   $total_price = isset($fetch_total_price['total_price']) ? $fetch_total_price['total_price'] : 0;
?>

<script>
   var totalpr = <?php echo $total_price; ?>;
   paypal.Buttons({
      style:{
         color:'blue',
         shape:'pill',
